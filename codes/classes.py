...
class observation:
    def __init__(self, ligne):
        ...
        self.heure_temperature_mini = convert_heure(
            heure_temperature_mini, self.date)
        self.heure_temperature_maxi = convert_heure(
            heure_temperature_maxi, self.date)
        ...
        if not self.vitesse_vent_instant_maxi == '':
            self.vitesse_vent_instant_maxi = convert_float(
                self.vitesse_vent_instant_maxi)*1.944
        self.heure_vent_instant_maxi = convert_heure(
            self.heure_vent_instant_maxi, self.date)
        self.humidite = convert_float(self.humidite)
        self.humidite_mini = convert_float(self.humidite_mini)
        self.heure_humidite_mini = convert_heure(
            self.heure_humidite_mini, self.date)
    ...
    def plafond(self):
        nuages = [(convert_int(self.n1), self.c1, self.b1), (convert_int(self.n2), self.c2, self.b2),
                  (convert_int(self.n3), self.c3, self.b3), (convert_int(self.n4), self.c4, self.b4)]
        nebu_max = 0
        couche_max = None
    ...
    def a_donnees_manquantes(self):
        tab = [self.q_hauteur_precipitation, self.q_duree_precipitation, self.q_temperature, self.q_dew_point, self.q_temperature_mini, self.q_heure_temperature_mini, self.q_temperature_maxi, self.q_heure_temperature_maxi, self.q_duree_gel, self.q_qfe, self.q_qnh, self.q_geopotentiel, self.q_qnh_mini, self.q_vitesse_vent,
               self.q_direction_vent, self.q_vitesse_vent_instant_maxi, self.q_direction_vent_instant_maxi, self.q_heure_vent_instant_maxi, self.q_humidite, self.q_humidite_mini, self.q_heure_humidite_mini, self.q_humidite_maxi, self.q_heure_humidite_maxi, self.q_nebulosite, self.q_temps_present, self.q_visi]
